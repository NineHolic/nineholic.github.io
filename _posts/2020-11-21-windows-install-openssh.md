---
layout: post
title: OpenSSH 在 Windows 上的使用
categories: [Windows]
description: OpenSSH 在 Windows 上的使用
keywords: OpenSSH, Windows
mermaid: false
sequence: false
flow: false
mathjax: false
...
默认加密方式为 rsa，使用参数 -t 指定 rsa 加密方式先在本地生成秘钥

![image-20201113154858419](https://fastly.jsdelivr.net/gh/FlyNine/cloudimage/win/image-20201113154858419.png)

(2)、将公钥上传到服务器 .ssh 目录下

PowerShell 中没有 ssh-copy-id 命令，手动将本地的 .ssh 目录上传到 windows 服务器用户目录下（避免文件权限问题），删除 id_rsa 和 known_hosts 并将 id_rsa.pub 重命名为 authorized_keys
